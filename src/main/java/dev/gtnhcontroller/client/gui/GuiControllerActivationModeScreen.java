...
public final class GuiControllerActivationModeScreen extends GuiScreen implements ControllerConfigurationScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(
            new GuiButton(SNEAK, centerX - 155, 74, 150, 20, modeLabel("Sneak", Config.sneakActivationMode)));
        buttonList.add(
            new GuiButton(SPRINT, centerX + 5, 74, 150, 20, modeLabel("Sprint", Config.sprintActivationMode)));
        buttonList.add(
            new GuiButton(ATTACK, centerX - 155, 100, 150, 20, modeLabel("Attack", Config.attackActivationMode)));
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            case SWIM:
                Config.swimActivationMode =
                    Config.swimActivationMode == ActivationMode.HOLD ? ActivationMode.TOGGLE : ActivationMode.HOLD;
                break;
            case SNEAK:
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(
            fontRendererObj,
            "HOLD follows the physical input.",
            width / 2,
            132,
            0xC0C0C0);
        drawCenteredString(
            fontRendererObj,
            "TOGGLE latches. PRESS emits one game tick.",
            width / 2,
            144,
            0xC0C0C0);
        drawCenteredString(
            fontRendererObj,
            "PRESS is mainly useful for Attack or Use.",
            width / 2,
            156,
            0xC0C0C0);
        drawCenteredString(
    ...
    private static String modeLabel(String label, ActivationMode mode) {
        String color = mode == ActivationMode.HOLD ? "\u00A7e"
            : mode == ActivationMode.TOGGLE ? "\u00A7a" : "\u00A7b";
